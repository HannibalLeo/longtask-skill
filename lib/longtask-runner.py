...
def validate_spec_contract(meta: dict[str, Any], phases: list[Phase]) -> list[str]:
    ...
    for key in ("final_verify_cmd", "final_smoke_cmd"):
    ...
    if not meta.get("final_verify_cmd") and not meta.get("final_smoke_cmd") and meta.get("final_gate") != "none":
        errors.append("missing final gate: set final_verify_cmd/final_smoke_cmd or final_gate: none")
    return errors
...
def dry_run(spec_path: Path, meta: dict[str, Any], phases: list[Phase]) -> int:
    ...
    print(
        ...
        f"final_gate={meta.get('final_gate', '<required>')}"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run a phased longtask spec.")
